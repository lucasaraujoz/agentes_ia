...
class App():
    def __init__(self):
        ...
        self.importarTiposLixos()

    '''
        monitorar lixeira em reciclaveis e organicos
    '''
    def importarTiposLixos(self):
        self.organicos.clear()
        self.reciclaveis.clear()
        for lixo in self.mapa.lixos:
            if(lixo.valor=="R"):
                self.reciclaveis.append(lixo)
            else:
                self.organicos.append(lixo)
    def importarObjetivos(self):
        if (len(self.objetivos) > 0):
            self.objetivos.clear()
        self.importarTiposLixos()
        aux = []
        if(len(self.organicos)>0):
            aux.extend(self.organicos)
        else:
            aux.extend(self.reciclaveis)

        for o in aux:
            self.objetivos.append((o.x,o.y))
