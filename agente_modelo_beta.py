...
class App():
    ...
    def remover_lixo_se_existir(self,y,x):
        ...
        if(percep != '.'): #teoria se alguma percepção for == "*" ai eu verifico o que é.
            if self.agente.carregandoLixo()==False:
                for lixo in self.mapa.lixos:
                    if(x == lixo.x and y == lixo.y):
                        self.agente.status=True
                        self.mapa.lixos.remove(lixo)
                        self.goBack=False
                        self.agente.posicaoLixo = (x,y)
                        break
                    #self.irAteLixeira()
            else:
                print("entra aqui")
                if percep =='*':
                    print("lixo enquanto carrega lixo ")
                #print("opa passou por alguma coisa que n é . ")
                #print(f"passou peli lixo {lixo.x}, {lixo.y}")
    ...
    def irAteLixeira(self):
        ...
        if(self.goBack):
            #self.agente.status=False true enquanto n volta pra pos do primeiro lixo porem gravar posições dos lixos por onde passar
    ...
    def retornarPosInicial(self):
        if(self.agente.x==self.agente.posicaoLixo[0] and self.agente.y==self.agente.posicaoLixo[1]):
            ...
            self.ir_ate_posicao(self.agente.posicaoLixo[0], self.agente.posicaoLixo[1])
        #return True
    ...
    def loop(self):
        while True:
            if(len(self.mapa.lixos)==0 and self.agente.x==19 and self.agente.y==19):
                break
            os.system('clear')
            print(self.agente.x, self.agente.y, "status=",self.agente.carregandoLixo(), "goback=", self.goBack, "direcao=", self.direcao)
            print(f'posição ultimo lixo ={self.agente.posicaoLixo}, listadelixos = {self.objetivos}')
            self.drawMap()
            self.draw()
            self.zigZag()
            input()
